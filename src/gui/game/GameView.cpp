...
void GameView::OnDraw()
{
	...
	else if (showHud)
	{
		...
		if (alpha < 50)
			alpha = 50;
		std::stringstream sampleInfo;
		std::stringstream realEmpty;
		sampleInfo.precision(2);
		if (sample.particle.type&&c->IsValidElement(sample.particle.type))
		{
			...
			if (sample.particle.type == PT_PHOT || sample.particle.type == PT_BIZR || sample.particle.type == PT_BIZRG || sample.particle.type == PT_BIZRS || sample.particle.type == PT_FILT || sample.particle.type == PT_BRAY)
				wavelengthGfx = ctype;
			if (c->IsValidElement(sample.particle.type)){
				if (showDebug&&sample.particle.element == 0)
				{
					...
					sampleInfo << ", Pressure: " << std::fixed << sample.AirPressure << " N/m^2";
				}
				else if (!showDebug&&sample.particle.element == 0)
				{
				...
				else if (sample.particle.element == 1){
					if (sample.particle.type == PT_HCBN || sample.particle.type == PT_OIL || sample.particle.type == PT_PRFN){
						stringstream chemName;
						int H = sample.particle.hydrogen;
						int C = sample.particle.carbon;
						int Cl = sample.particle.chlorine;
						int alcohol = sample.particle.alcohol;
						int bonds = sample.particle.bonds;
						int bond = sample.particle.bond;
						bool phenyl = sample.particle.phenyl;
						bool benzyl = sample.particle.benzyl;

						if (Cl > 0) chemName << Cl << "-chloro-";
						if (bond >= 2) chemName << bond << "-";
						switch (C){
						case 1:
							chemName << "meth";
							break;
						...
						case 50:
							...
							break;
						default:
							if (bond == 0)chemName << "kerosene " << C << "-carbon";
							else chemName << "parrafin " << C << "-carbon";
							break;
						}
						if ((C >= 1 && C <= 15) || C == 20 || C == 30 || C == 40 || C == 50){
							if ((bonds == 2 * C + 2 && C > 1) || (C == 1)){
								if (alcohol == 0)chemName << "ane";
								else if (alcohol == 1)chemName << "anol";
								else chemName << "an-" << alcohol << "-ol";
							}
							else if (bonds == 2 * C - 2 && C > 1){
								if (alcohol == 0)chemName << "yne";
								else if (alcohol == 1) chemName << "ynol";
								else chemName << "yn-" << alcohol << "-ol";
							}
							else {
								if (alcohol == 0)chemName << "ene";
								else if (alcohol == 1) chemName << "enol";
								else chemName << "en-" << alcohol << "-ol";
							}
						}
						else{
							if (sample.particle.type == PT_HCBN) chemName << " gas";
							else if (sample.particle.type == PT_PRFN) chemName << " wax";
							else if (sample.particle.type == PT_OIL) chemName << " oil";
							if (alcohol >= 1)chemName << " alcohol";
						}
						sampleInfo << chemName.str();
						if (C == 1){
							if (alcohol == 0)sampleInfo << ": CH" << H;
							else sampleInfo << ": CH " << H - 1;
						}
						else{
							if (alcohol == 0)sampleInfo << ": C" << C << "H" << H;
							else sampleInfo << ": C" << C << "H" << H - 1;
						}
						if (Cl > 0) sampleInfo << "Cl";
						if (alcohol > 0) sampleInfo << "OH";
						if (bonds == 2 * C - 2 && C > 1)sampleInfo << ", Triple Bond: C" << bond;
						else {
							if (bonds < 2 * C + 2 && C > 1)sampleInfo << ", Double Bond: C" << bond;
						}
						sampleInfo << ", State: ";
						if (sample.particle.type == PT_HCBN)sampleInfo << "Gasseous";
						else if (sample.particle.type == PT_OIL)sampleInfo << "Oily";
						else if (sample.particle.type == PT_PRFN)sampleInfo << "Waxy";
						sampleInfo << ", Energy: " << sample.particle.energy;
						sampleInfo << ", Temp: " << std::fixed << sample.particle.temp - 273.15f << " C";
						sampleInfo << ", Pressure: " << std::fixed << sample.AirPressure << " N/m^2";
					}
					else if (sample.particle.type == PT_ALCO){
						int C = sample.particle.carbon;
						int H = sample.particle.hydrogen;
						int bonds = H + (sample.particle.chlorine > 0) - 1;
						int alcohol = sample.particle.alcohol;
						stringstream alcName;
						if (alcohol > 0 && ((C >= 1 && C <= 15) || C == 20 || C == 30 || C == 40 || C == 50))alcName << alcohol << "-";
						switch (C){
						case 1:
							alcName << "meth";
							break;
						...
						case 50:
							...
							break;
						default:
							alcName << C << "-carbon alcohol bonded on C" << alcohol;
							break;
						}
						if ((C >= 1 && C <= 15) || C == 20 || C == 30 || C == 40 || C == 50){
							if (bonds == 2 * C - 2){
								if (alcohol > 0)alcName << "yn-" << alcohol << "-ol";
								else alcName << "ynol";
							}
							else if (bonds == 2 * C + 2){
								if (alcohol > 0)alcName << "an-" << alcohol << "-ol";
								else alcName << "anol";
							}
							else{
								if (alcohol > 0) alcName << "en-" << alcohol << "-ol";
								else alcName << "enol";
							}
						}
						sampleInfo << alcName.str();
						sampleInfo << ", Temp: " << std::fixed << sample.particle.temp - 273.15f << " C";
						sampleInfo << ", Pressure: " << std::fixed << sample.AirPressure << " N/m^2";
					}
					else{
						switch (sample.particle.type){
						case PT_QARK:
							sampleInfo << sample.particle.flavour << " Quark, Energy: " << sample.particle.energy << " eV, Mass: " << sample.particle.mass << " MeV/c^2";
							break;
						case PT_PROT:
							sampleInfo << "Proton, Energy: " << sample.particle.energy << " eV, Mass: " << sample.particle.mass << " MeV/c^2";
							break;
						case PT_NEUT:
							sampleInfo << "Neutron, Energy: " << sample.particle.energy << " eV, Mass: " << sample.particle.mass << " MeV/c^2";
							break;
						case PT_ELEC:
							sampleInfo << "Electron, Energy: " << sample.particle.energy << " eV, Mass: " << sample.particle.mass << " MeV / c^2";
							break;
						case PT_PHOT:
							sampleInfo << "Photon, Mass: 10e-18 eV/c^2";
							break;
						case PT_GRVT:
							sampleInfo << "Graviton, Energy: " << sample.Gravity << " m/s^2, Mass: 10e-18 eV/c^2";
							break;
						case PT_GLUON:
							sampleInfo << "Gluon, Energy: " << sample.particle.energy << " eV, Mass: " << sample.particle.mass << " eV/c^2";
							break;
						case PT_CLRN:
							sampleInfo << "Chlorine: Cl";
							if (sample.particle.life == 2)sampleInfo << "2";
							break;
						case PT_H2:
							sampleInfo << "Hydrogen: Bonding Potential: " << sample.particle.life;
							break;
						case PT_CRBN:
							sampleInfo << "Carbon: C" << sample.particle.life;
							break;
						case PT_HXDE:
							sampleInfo << "Hydroxide Ion";
							break;
						case PT_O2:
							sampleInfo << "Oxygen";
							break;
						case PT_BNZN:
							if (sample.particle.carbon == 6 && sample.particle.hydrogen == 6)sampleInfo << "Benzene: C6H6";
							else if (sample.particle.carbon == 7 && sample.particle.hydrogen == 9)sampleInfo << "Phenyl Group";
							else if (sample.particle.carbon == 9 && sample.particle.hydrogen == 11)sampleInfo << "Benzyl Group";
							else sample.particle.type = PT_BNZN;
							break;
						case PT_StrongInteraction:
							sampleInfo << "Gluon Field";
							break;
						default:
							sampleInfo << c->ElementResolve(sample.particle.type, sample.particle.ctype);
							if (wavelengthGfx)
			...
			{
				sampleInfo << "Empty, Pressure: " << std::fixed << sample.AirPressure << " N/m^2";
			}
			else
			{
				sampleInfo << "Empty";
		...
		realEmpty << "Empty, Pressure: " << std::fixed << sample.AirPressure << " N/m^2, Ambient Temperature: " << std::fixed << sample.AirTemperature << " C";
		int textWidth = Graphics::textwidth((char*)sampleInfo.str().c_str());
		int altTextWidth = Graphics::textwidth((char*)realEmpty.str().c_str());
		std::string output = sampleInfo.str();
		std::string glitch = "(";
		if (output.find(glitch)!=std::string::npos&&sizeof(output)>=60){
			g->fillrect(XRES - 20 - altTextWidth, 12, altTextWidth + 8, 15, 0, 0, 0, alpha*0.5f);
			g->drawtext(XRES - 16 - altTextWidth, 16, (const char*)realEmpty.str().c_str(), 255, 255, 255, alpha*0.75f);
		}
		else{
	...
	if (showHud && introText < 51)
	{
		...
		g->fillrect(12, 28, textWidth + 8, 15, 0, 0, 0, alpha*0.5);
		g->drawtext(16, 32, (const char*)fpsInfo.str().c_str(), 32, 216, 255, alpha*0.75);
	}
