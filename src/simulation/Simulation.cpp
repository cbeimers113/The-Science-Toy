	...
	switch (t)
		{
			...
			case PT_GLUON:
			{
				...
				parts[i].vx = 10.0f*cosf(a);
				parts[i].vy = 10.0f*sinf(a);
				parts[i].element = 1;
				parts[i].mass = 2e-4f;
				break;
			}
			case PT_PHOT:
			{
				...
				parts[i].vx = 15.0f*cosf(a);
				parts[i].vy = 15.0f*sinf(a);
			...
			{
				float a = (rand() % 36)* 0.17453f;
				parts[i].vx = 8.0f*cosf(a);
				parts[i].vy = 8.0f*sinf(a);
				int c = rand() % 3;
				parts[i].colourCharge = c==0?0xff0000:c==1?0xff00:0xff;
				int q = rand() % 100;
				if (q >= 75){
					parts[i].flavour = "Up";
					parts[i].mass = 2.3f+(rand()%12-6)/10.0f;
					parts[i].dcolour = 0xff006600;
					parts[i].life = 150;
