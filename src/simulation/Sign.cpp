...
std::string sign::getText(Simulation *sim)
{
	...
	if(signText[0] && signText[0] == '{')
	{
		if (!strcmp(signText,"{p}"))
		{
			...
			sprintf(buff, "Pressure: %3.2f N/m^2", pressure);  //...pressure
		}
		else if (!strcmp(signText,"{t}"))
		{
			if (x>=0 && x<XRES && y>=0 && y<YRES && sim->pmap[y][x])
				sprintf(buff, "Temp: %4.2f C", sim->parts[sim->pmap[y][x]>>8].temp-273.15f);  //...temperature
			else
				sprintf(buff, "Temp: 0.00");  //...temperature
		}
		else if (!strcmp(signText, "{h}")&&sim->aheat_enable){
			float aHeat = 0.0f;
			if (x >= 0 && x<XRES && y >= 0 && y<YRES)
				aHeat = sim->hv[y / CELL][x / CELL];
			sprintf(buff, "Ambient Heat: %4.2f C", aHeat - 273.15f);
		}
		else
		{
